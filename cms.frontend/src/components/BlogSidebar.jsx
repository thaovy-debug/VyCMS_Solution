...
function BlogSidebar({ activeCategory, onFilterChange }) {
    ...
    return (
        <div className="card p-3 shadow-sm border-0" style={{ borderRadius: '15px' }}>
            <h6 className="font-weight-bold text-uppercase mb-3" style={{ color: 'var(--thieuhoa-primary)', letterSpacing: '1px' }}>
            ...
            <div className="list-group list-group-flush">
                ...
                <button
                    ...
                    style={{ borderRadius: '8px', transition: 'all 0.2s', backgroundColor: activeCategory === null ? '#f8f9fa' : 'transparent', color: activeCategory === null ? 'var(--thieuhoa-primary)' : 'inherit' }}
                >
                    <i className={`fas fa-chevron-right mr-2 small ${activeCategory === null ? 'opacity-100' : 'opacity-0'}`} style={{ color: 'var(--thieuhoa-primary)' }}></i>
                    Tất cả bản tin
                </button>

                {loading ? (
                    <div className="text-center py-3">
                        <div className="spinner-border spinner-border-sm text-primary" role="status" style={{ color: 'var(--thieuhoa-primary)' }}></div>
                    </div>
                ) : (
                    categories.map(cat => (
                        ...
                        <button
                            ...
                            style={{ borderRadius: '8px', transition: 'all 0.2s', fontSize: '15px', backgroundColor: activeCategory === cat.id ? '#f8f9fa' : 'transparent', color: activeCategory === cat.id ? 'var(--thieuhoa-primary)' : 'inherit' }}
                        >
                            <i className={`fas fa-chevron-right mr-2 small ${activeCategory === cat.id ? 'opacity-100' : 'opacity-0'}`} style={{ color: 'var(--thieuhoa-primary)' }}></i>
                            {cat.name}
                        </button>
                    ))
                )}
